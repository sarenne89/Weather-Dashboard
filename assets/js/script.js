    ...
    for (let i = 0; i < 5; i++) {
      if (retrievedHistory[i] != null) {
        previousSearch = $("<button>")
          .val(retrievedHistory[i])
          .text(retrievedHistory[i])
          .attr({
        ...
        fetch(weatherQueryURL)
          ...
          .then(function (data) {
            ...
            if (data.weather[0].description === "broken clouds") {
              today.attr("class", "mt-3 p-2 brokenClouds");
            }
          ...
          .then(function (data2) {
            for (let index = 0; index < data2.list.length; index++) {
              if (data2.list[index].dt_txt.includes("12:00:00")) {
                forecastCard = $("<div>").addClass(
                  "card col-12 col-md-2 mx-md-auto mx-2 my-2 bg-dark text-light"
                );
                forecastDate = $("<h5>").text(
                  dayjs.unix(data2.list[index].dt).format("DD-MM-YYYY")
                );
                forecastDate.addClass("text-center");
                forecastTime = $("<p>")
                  .text("@12pm")
                  .attr("class", "mb-2 text-center");
                forecastTemp = $("<p>").text(
                  "Temp: " +
                    (data2.list[index].main.temp -= 273.15).toFixed(1) +
                    "°C"
                );
                forecastWind = $("<p>").text(
                  "Wind: " + data2.list[index].wind.speed.toFixed(1) + " m/s"
                );
                forecastHumidity = $("<p>").text(
                  "Humidity: " + data2.list[index].main.humidity + "%"
                );
